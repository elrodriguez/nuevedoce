<div>
    <div class="card mb-g rounded-top">
        <div class="card-header">
            <div class="input-group bg-white shadow-inset-2">
                ...
                <input wire:keydown.enter="quotasSearch" wire:model.defer="search" type="text" class="form-control border-left-0 bg-transparent pl-0" placeholder="{{__('lend::labels.lbl_type_here')}}">
                <div class="input-group-append">
                    <button wire:click="quotasSearch" class="btn btn-default waves-effect waves-themed" type="button">@lang('lend::buttons.btn_search')</button>
                    @can('prestamos_cuotas_nuevo')
                        <a href="{{ route('lend_quota_create') }}" class="btn btn-success waves-effect waves-themed" type="button">@lang('lend::buttons.btn_new')</a>
                    @endcan
                </div>
            </div>
        </div>
        <div class="card-body p-0">
            <table class="table m-0">
                ...
                <tr>
                    ...
                    <th>@lang('lend::labels.lbl_amount')</th>
                ...
                @foreach($quotas as $key => $quota)
                    <tr>
                        ...
                        <td class="text-center tdw-50 align-middle">
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu" style="position: absolute; will-change: top, left; top: 35px; left: 0px;" x-placement="bottom-start">
                                    @can('prestamos_cuotas_editar')
                                        <a href="{{ route('lend_quota_edit', $quota->id) }}" class="dropdown-item">
                                            <i class="fal fa-pencil-alt mr-1"></i>@lang('lend::buttons.btn_edit')
                                        </a>
                                    @endcan
                                    @can('prestamos_cuotas_eliminar')
                                        <div class="dropdown-divider"></div>
                                        <button onclick="confirmDelete({{ $quota->id }})" type="button" class="dropdown-item text-danger">
                        ...
                        <td class="align-middle">{{ $quota->amount }}</td>
        ...
        <div class="card-footer  pb-0 d-flex flex-row align-items-center">
            <div class="ml-auto">{{ $quotas->links() }}</div>
        </div>
    </div>
    <script type="text/javascript">
        ...
        document.addEventListener('len-quota-delete', event => {
