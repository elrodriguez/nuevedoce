    ...
    <div class="card mb-g rounded-top">
        <div class="card-header">
            <div class="input-group bg-white shadow-inset-2">
                ...
                <input wire:keydown.enter="interestsSearch" wire:model.defer="search" type="text" class="form-control border-left-0 bg-transparent pl-0" placeholder="{{__('lend::labels.lbl_type_here')}}">
                <div class="input-group-append">
                    <button wire:click="interestsSearch" class="btn btn-default waves-effect waves-themed" type="button">@lang('lend::buttons.btn_search')</button>
                    @can('prestamos_intereses_nuevo')
                        <a href="{{ route('lend_interest_create') }}" class="btn btn-success waves-effect waves-themed" type="button">@lang('lend::buttons.btn_new')</a>
                    @endcan
                </div>
            </div>
        </div>
        <div class="card-body p-0">
            <table class="table m-0">
                ...
                <tr>
                    ...
                    <th class="text-center">@lang('lend::labels.lbl_actions')</th>
                    <th>@lang('lend::labels.lbl_description')</th>
                    <th>@lang('lend::labels.lbl_value')</th>
                ...
                @foreach($interests as $key => $interest)
                    <tr>
                        ...
                        <td class="text-center tdw-50 align-middle">
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu" style="position: absolute; will-change: top, left; top: 35px; left: 0px;" x-placement="bottom-start">
                                    @can('prestamos_intereses_editar')
                                        <a href="{{ route('lend_interest_edit', $interest->id) }}" class="dropdown-item">
                                            <i class="fal fa-pencil-alt mr-1"></i>@lang('lend::buttons.btn_edit')
                                        </a>
                                    @endcan
                                    @can('prestamos_intereses_eliminar')
                                        <div class="dropdown-divider"></div>
                                        <button onclick="confirmDelete({{ $interest->id }})" type="button" class="dropdown-item text-danger">
                        ...
                        <td class="align-middle">{{ $interest->description }}</td>
                        <td class="align-middle">{{ $interest->value }}</td>
                        <td class="text-center align-middle">
                            @if($interest->state)
                                <span class="badge badge-success">{{ __('lend::labels.lbl_active') }}</span>
                            @else
                                <span class="badge badge-danger">{{ __('lend::labels.lbl_inactive') }}</span>
                            @endif
                        </td>
                    </tr>
                @endforeach
        ...
        <div class="card-footer  pb-0 d-flex flex-row align-items-center">
            <div class="ml-auto">{{ $interests->links() }}</div>
        </div>
    </div>
