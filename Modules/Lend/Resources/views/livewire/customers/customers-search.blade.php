        ...
        <div class="card-body">
            <div class="form-row needs-validation input-group {{ $errors->any()?'was-validated':'' }}" novalidate="">
                <div class="input-group bg-white shadow-inset-2">
                    <input wire:keydown.enter="searchPerson" wire:model="number_search" maxlength="11" type="text" class="form-control border-left-1 bg-transparent pl-1" id="number_search" required="" placeholder="{{__('lend::labels.lbl_enter_identity_document_number')}}">
                    @error('number_search')
                    <div class="invalid-feedback-2">{{ $message }}</div>
                    @enderror
                    <div class="input-group-append">
                        <button wire:click="searchPerson" wire:loading.attr="disabled" type="button" class="btn btn-info ml-auto waves-effect waves-themed">@lang('lend::buttons.btn_search')</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script type="text/javascript">
        ...
        document.addEventListener('ser-customers-search_a', event => {
            ...
            let box = bootbox.confirm({
                ...
                buttons:
                    {
                        confirm:
                            {
                                label: '{{__('lend::buttons.btn_yes')}}',
                                className: 'btn-danger shadow-0'
                            },
                        cancel:
                            {
                                label: '{{__('lend::buttons.btn_not')}}',
                ...
                callback: function(result) {
                    if(result){
                        let url = "{{ route('lend_customers_create', ':id') }}";
        ...
        document.addEventListener('ser-customers-search_c', event => {
            ...
            let box = bootbox.confirm({
                ...
                callback: function(result) {
                    if(result) {
                        let url = "{{ route('lend_customers_edit', ':id') }}";
