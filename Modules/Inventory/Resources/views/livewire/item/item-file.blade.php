<div wire:ignore>
    <form action="{{ route('inventory_item_upload_file') }}" method="POST" class="dropzone needsclick" id="file-upload" enctype="multipart/form-data" style="min-height: 7rem; height: 120%;">
        ...
        // try {
            var myDropzone = new Dropzone('#file-upload', {
                ...
                init: function() {
                    thisDropzone = this;
                    $(".dz-preview").remove();
                    var data_aaa = "{{ json_encode($result) }}";
                ...
                success: function (file, response) {
                    if(response.result == "OK"){
                        ...
                        let box = bootbox.alert({
                            ...
                            message: "<span><strong>{{__('inventory::labels.lbl_excellent')}}... </strong>{{__('inventory::labels.msg_0004')}}</span>",
                            centerVertical: true,
                        ...
                        let box = bootbox.alert({
                            ...
                            message: "<span><strong>{{ __('inventory::labels.went_wrong') }}... </strong> {{__('inventory::labels.msg_0005')}}</span>",
                            centerVertical: true,
                            className: "modal-alert",
                            closeButton: false,
                            callback: function () {
                ...
                error: function (file, response) {
                    ...
                    let box = bootbox.alert({
                        title: "<i class='fal fa-check-circle text-warning mr-2'></i> <span class='text-warning fw-500'>{{ __('inventory::labels.error') }}!</span>",
                        message: "<span><strong>{{ __('inventory::labels.error') }}... </strong>"+response+"</span>",
                        centerVertical: true,
                        className: "modal-alert",
                        closeButton: false,
                        callback: function () {
                ...
                removedfile: function (file, data) {
                    ...
                    //Ruta a eliminar
                    @this.deleteFile(idfile, narchivo, url_file);
                    var _ref;
    ...
    document.addEventListener('set-file-delete', event => {
        ...
        if(res == 'OK') {
            ...
            let box = bootbox.alert({
                ...
                message: "<span><strong>{{ __('inventory::labels.excellent') }}... </strong>{{ __('inventory::labels.msg_0006') }}</span>",
                centerVertical: true,
                className: "modal-alert",
                closeButton: false
            });
            ...
            let box = bootbox.alert({
                title: "<i class='fal fa-check-circle text-warning mr-2'></i> <span class='text-warning fw-500'>{{ __('inventory::labels.error') }}!</span>",
                message: "<span><strong>{{ __('inventory::labels.went_wrong') }}... </strong>{{ __('inventory::labels.msg_0007') }}</span>",
                centerVertical: true,
                className: "modal-alert",
                closeButton: false
            });
