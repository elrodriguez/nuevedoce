<div>
    <!-- Modal -->
    <div wire:ignore.self class="modal fade" id="modalUserEstablisment" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-body">
                    ...
                    <div class="mt-3">
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" wire:model.defer="main" id="defaultChecked">
                            <label class="custom-control-label" for="defaultChecked">Trabajará aquí hoy?</label>
                        </div>
                    </div>
                    <div class="mt-3">
                        <button wire:click="save" type="button" class="btn btn-primary">{{ __('labels.add') }}</button>
                    </div>
                                    ...
                                    @foreach ($user_establishments as $k => $item)
                                        <tr>
                                            <td class="align-middle text-center" width="10%">
                                                <button class="btn btn-default btn-sm btn-icon rounded-circle waves-effect waves-themed" wire:click="delete({{ $item->id }})" type="button">
                                            ...
                                            <td>
                                                <div class="custom-control custom-switch">
                                                    ...
                                                    @if($item->main)
                                                        <input wire:change="inactiveMain({{ $item->id }})" {{ $item->main ? 'checked' : '' }} type="checkbox" class="custom-control-input" id="customSwitch2{{ $k }}">
                                                        <label class="custom-control-label" for="customSwitch2{{ $k }}">{{ __('labels.active') }}</label>
                                                    @else
                                                        <input wire:change="activeMain({{ $item->id }})" {{ $item->main ? 'checked' : '' }} type="checkbox" class="custom-control-input" id="customSwitch2{{ $k }}">
                                                        <label class="custom-control-label" for="customSwitch2{{ $k }}">{{ __('labels.inactive') }}</label>
                                                    @endif
                                                </div>
                                            </td>
                                        </tr>
