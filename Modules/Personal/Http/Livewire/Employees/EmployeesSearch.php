...
use Modules\Personal\Http\Controllers\EmployeeController;
...
class EmployeesSearch extends Component
{
    ...
    {
        return view('personal::livewire.employees.employees-search');
    }
    ...
    public function searchPerson(){
        ...
        foreach ($this->person_search as $key => $personSearch){
            ...
            //dd($this->person_id);
            $this->employee_search = PerEmployee::where('person_id', $this->person_id)->get();
        ...
        if($encuentra == ''){
            $this->dispatchBrowserEvent('per-employees-search_a', ['msg' => Lang::get('personal::labels.msg_search_not'), 'numberPerson' => $this->number_search]);
        }elseif($encuentra != '' && $encuentra_e == ''){
            $this->dispatchBrowserEvent('per-employees-search_b', ['msg' => Lang::get('personal::labels.msg_search_ok_a'), 'personId' => $this->person_id]);
        }else{
            $this->dispatchBrowserEvent('per-employees-search_c', ['msg' => Lang::get('personal::labels.msg_search_ok_b'), 'employeeId' => $this->employee_id]);
        }
    }
